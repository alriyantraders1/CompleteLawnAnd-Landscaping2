...
export default function Quote() {
  ...
  return (
    <section id="quote" className="relative w-full py-24 px-6 md:px-10 overflow-hidden">
      <img src="https://readdy.ai/api/search-image?query=aerial%20view%20of%20perfectly%20manicured%20green%20lawn%20with%20geometric%20mowing%20patterns&width=1920&height=900&seq=quote001&orientation=landscape"
        alt="Lawn background" className="absolute inset-0 w-full h-full object-cover object-top" />
      ...
      <div className="relative z-10 max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            {[
              ...
              <div key={item.label} className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <a href={item.href} className="text-white text-sm mt-0.5 block hover:text-green-300 transition-colors cursor-pointer">{item.value}</a>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-white rounded-3xl p-8 md:p-10">
          ...
          {status === 'success' ? (
            ...
            <form name="quote" method="POST" data-netlify="true" onSubmit={handleSubmit} className="flex flex-col gap-5">
              ...
              <div>
                ...
                <select name="service" className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-800 focus:outline-none focus:border-green-500 transition-colors bg-white cursor-pointer">
              ...
              <div>
                ...
                <input type="text" name="property_address" placeholder="123 Main St, Carnegie, PA"
                  className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-800 focus:outline-none focus:border-green-500 transition-colors" />
              </div>
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1.5">Additional Details</label>
                <textarea name="message" rows={3} maxLength={500} placeholder="Tell us more about your lawn or project..."
              ...
              <button type="submit" disabled={status === 'sending'}
                ...
                {status === 'sending' ? <><i className="ri-loader-4-line animate-spin"></i> Sending...</> : <><i className="ri-send-plane-line"></i> Request My Free Quote</>}
              </button>
            </form>
          )}
        </div>
